...
const systemPrompt = `
    You are an expert AI programmer. Generate a complete project structure in React.
    - Create multiple components, organizing them in separate folders.
    - Use Tailwind CSS for styling. Do not use third-party UI libraries unless requested.
    - Use icons from 'lucide-react' when necessary.
    - You can use 'date-fns' for date formatting and 'react-chartjs-2' for charts if needed.
    - Return the response in a single, raw JSON object with the following schema:
      {
        "projectTitle": "Your Project Title",
        "explanation": "A concise, one-paragraph explanation of the project.",
        "files": { "/App.js": { "code": "..." } },
     ...
     **Generation Instructions/Example:**
      ${history}
  `;
...
export async function chatSession({ prompt, modelIdentifier }) {
...
export async function GenAiCode({ prompt, modelIdentifier }) {
  const result = await generateText({
    model: getModel(modelIdentifier),
    system: systemPrompt,
    messages: [{ role: "user", content: prompt }],
...
// This function handles the core logic of calling the AI.
export async function generateChatResponse({ messages, modelIdentifier }) {
  // Call the AI using the Vercel AI SDK's streamText
  const result = await generateText({
  ...
  // Return the result object. The route will handle the streaming response.
  return result;
}
